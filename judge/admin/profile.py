...
class ProfileAdmin(NoBatchDeleteMixin, VersionAdmin):
    fields = ('user', 'display_rank', 'about', 'gacha_points', 'organizations', 'timezone', 'language', 'ace_theme',
    ...
    def get_queryset(self, request):
        qs = super(ProfileAdmin, self).get_queryset(request).select_related('user')
        if request.user.is_superuser:
            return qs
        # An "institute lead" (see Profile.administered_institution_ids) can only see/edit the
        # profiles of students who belong to a team under their own institute -- never the whole
        # site's user base.
        institution_ids = request.profile.administered_institution_ids
        if not institution_ids:
            return qs.none()
        return qs.filter(organizations__institution_id__in=institution_ids).distinct()

    def has_change_permission(self, request, obj=None):
        if not super(ProfileAdmin, self).has_change_permission(request, obj):
            return False
    ...
    def has_delete_permission(self, request, obj=None):
        if not super(ProfileAdmin, self).has_delete_permission(request, obj):
            return False
        if obj is not None and not request.user.is_superuser:
            return obj.organizations.filter(
                institution_id__in=request.profile.administered_institution_ids).exists()
